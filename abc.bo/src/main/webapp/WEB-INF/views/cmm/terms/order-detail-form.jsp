...
<script type="text/javascript">
	$(document).ready(function(){
		...
		<c:forEach var="cmTermsDetailList" items="${cmTermsDetailList}" varStatus="idx">
			abc.biz.cmm.terms.getOrderDetailArea("${cmTerms.getTermsSeq()}", "${idx.count}");
		</c:forEach>
		...
		$("#btnSaveOrder").off().on('click', function(){
			...
			abc.biz.cmm.terms.doAction('orderUpdateCheck');
		});
		//테이블 삭제 버튼
...
<form action="/cmm/terms/update-signup-order" id="orderForm" name="orderForm" method="post">
	...
	<input type="hidden" id="tabTitle" value="4">
	<!-- S : container -->
	<div class="container">
		<div class="content-box">
			...
			<div class="content-header">
				...
				<div class="fr">
					<div class="btn-group">
						...
						<a href="javascript:void(0)" id="reloadOrderForm" class="btn-sm btn-func"><i class="ico ico-refresh"></i>초기화</a>
						<!-- E : 20190114 수정 -->
					</div>
				</div>
			</div>
			...
			<table class="tbl-row">
				...
				<tbody>
					<tr>
						...
						<td class="input" colspan="3">
							<span class="date-box">
								<input type="text" id="termsApplyYmd" name="termsApplyYmd" data-role="datepicker" value='<fmt:formatDate value="${cmTerms.getTermsApplyYmd()}" pattern="${Const.DEFAULT_DATE_PATTERN}"/>' class="ui-cal js-ui-cal" title="시행일자 선택">
							</span>
						</td>
					</tr>
					<tr id="saveHide">
			...
			<div class="content-bottom">
				<!-- S : 20190114 수정 // 버튼 가이드 변경으로 마크업 수정 -->
				<a href="javascript:void(0)" id="btnSaveOrder" class="btn-lg btn-save">저장</a>
				<!-- E : 20190114 수정 -->
			</div>
			<!-- E : content-bottom -->
		</div>
	...
	<input type="hidden" name="orderPageCount" 		value="${param.pageCount}"/>
	<input type="hidden" name="orderListPageNum" 	value="${param.listPageNum}"/>
	<input type="hidden" name="orderTermsDtlCode" 	value="${param.searchTermsDtlCode}"/>
