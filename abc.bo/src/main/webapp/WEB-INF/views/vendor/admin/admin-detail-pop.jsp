...
<script type="text/javascript">
	$(function() {
		...
		abc.biz.vendor.admin.initAdminDetailData();

		// 이메일 상세정보
		var emailText = '<c:out value="${detailInfo.emailAddrText}"/>';
		abc.biz.vendor.admin.detailEmailSet(emailText);

		// 탭 클릭
		$("#tabDetail").on("tabsactivate", function(event, ui) {
			abc.biz.vendor.admin.tabHandler(event, ui);
		});
		...
		$("#authNo").change(function(){
			...
			abc.biz.vendor.admin.authGroupChange(authNo);
		});
		...
		$("#resetBtn").click(function(){
			abc.biz.vendor.admin.pswdReset();
		});

		<%-- 이메일 변경 시 세팅 --%>
		$("#selEmail").change(function(){
			var selEmailVal = $(this).val();
			abc.biz.vendor.admin.changeEmail(selEmailVal);
		});

		<%-- 관리자 정보 저장 --%>
		$("#saveBtn").click(function(){
			abc.biz.vendor.admin.updateAdmin();
		});
