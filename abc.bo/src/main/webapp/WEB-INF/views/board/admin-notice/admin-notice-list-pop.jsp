...
<script type="text/javascript">
	$(function() {
		<%-- 관리자 공지 목록 팝업 초기 세팅. --%>
		abc.biz.board.adminnotice.initAdminNoticePopSheet();

		<%-- 관리자 공지 목록 화면 호출 시 첫 검색, 대시보드에서 이동시 대시보드 URL 타도록 수정(검색, 상세팝업) --%>
		abc.biz.board.adminnotice.adminnoticePopDoAction("search", '<c:out value="${fromDash}" />');
		...
		$("#btnSearch").click(function(){
			abc.biz.board.adminnotice.adminnoticePopDoAction('search');
		});
	});
	...
	function adminnoticePopSheet_OnClick(Row, Col, Value, CellX, CellY, CellW, CellH) {
		if ( Row != 0) {
			if ( adminnoticePopSheet.ColSaveName(Col) == "notcTitleText" && Value != "" ) {
				...
				var url = "/board/admin-notice/read-detail-pop";
