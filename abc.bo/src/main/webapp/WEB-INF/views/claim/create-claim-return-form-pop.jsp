...
<script type="text/javascript">
	...
	function m_Completepayment( FormOrJson, closeEvent ) {
		...
		if(frm.res_cd.value == "0000") {
			...
			closeEvent();
			abc.biz.claim.create.registClaimReturn(); // 클레임 반품 등록
		} else {
...
<script type="text/javascript">
	$(function() {
		<%-- Const 지정 --%>
		abc.biz.claim.create.consts.ADD_DLVY_AMT_PYMNT_TYPE_ADD_PAYMENT = "${CommonCode.ADD_DLVY_AMT_PYMNT_TYPE_ADD_PAYMENT}"; // 추가배송비결제방법 : 추가결제
		abc.biz.claim.create.consts.ADD_DLVY_AMT_PYMNT_TYPE_FREE_COUPON = "${CommonCode.ADD_DLVY_AMT_PYMNT_TYPE_FREE_COUPON}"; // 추가배송비결제방법 : 무료쿠폰
		
		abc.biz.claim.create.SITE_NO_ART	= "${Const.SITE_NO_ART}";
		abc.biz.claim.create.SITE_NO_OTS	= "${Const.SITE_NO_OTS}";
		
		abc.biz.claim.create.ABC_FO_URL = "${Const.SERVICE_DOMAIN_ART_FO}";
		abc.biz.claim.create.OTS_FO_URL = "${Const.SERVICE_DOMAIN_OTS_FO}";
		
		<%-- 접수 초기 회수지 기본정보 set(주문정보 배송지 이용) --%>
		abc.biz.claim.create.setDefaultAddrInfo();
		
		<%-- 일괄적용 처리용 콤보박스 변경 --%>
		$("#codeDivSelect").change(function() {
			abc.biz.claim.create.changeComboForBatchApply();
		});
		
		<%-- 일괄적용 --%>
		$("#batchApplyBtn").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.batchApplyProduct();
		});
		
		<%-- 클레임 반품 접수 --%>
		$("#claimRegistBtn").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.registClaimReturnPreProc();
		});
	    ...
	    $("#retrievalPostSearchBtn").click(function() {
	        abc.postPopup(abc.biz.claim.create.setRetrievalPost);
	    });
	
		<%-- 배송주소와 동일 체크박스 클릭 --%>
		$("#chkDeliveryModule").click(function() {
			abc.biz.claim.create.toggleSameDelivery(this);
	    });
		...
		$("#chkClaimBatchAll").click(function() {
			abc.biz.claim.create.toggleCheckAll("chkClaimBatchAll", "chkTarget");
	    });
		
		<%-- 배송비 환불금차감, 선결제 선택 시 폼 제어 --%>
		$("#radioPayment01, #radioPayment02").click(function() {
			abc.biz.claim.create.toggleDeliveryAmtForm(this);
	    });
		...
		$("#adminBoPop").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.adminBoPop(this);
        });
		
		$("#orderBoPop").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.orderBoPop(this);
        });
		
		$("a[name='prdtBoPop']").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.prdtBoPop(this);
        });
		
		$("a[name='prdtFo']").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.prdtFo(this);
        });
		
		$("a[name='vndrBoPop']").click(function(e) {
			e.preventDefault();
			abc.biz.claim.create.vndrBoPop(this);
		});
		
		<%-- 2020.05.07: 주문상세에서 넘어오는 비회원 AS반풉접수 여부를 따져 화면 set func --%>
		abc.biz.claim.create.asReturn();
	});
</script>
...
<body class="window-body">
	<div class="window-wrap">
		...
		<div class="window-content">
			...
			<%-- 배송지 정보 : 반품에서는 배송지 정보를 사용하지 않으나 저장 시 회수지 정보로 임의로 set --%>
			<input type="hidden" id="rcvrName" name="rcvrName" value="${orderInfo.rcvrName}"/>
			<input type="hidden" id="rcvrHdphnNoText" name="rcvrHdphnNoText" value="${fn:trim(fn:replace(orderInfo.rcvrHdphnNoText, '-', ''))}"/>
			<input type="hidden" id="rcvrPostCodeText" name="rcvrPostCodeText" value="${fn:trim(fn:replace(orderInfo.rcvrPostCodeText, '-', ''))}"/>
			<input type="hidden" id="rcvrPostAddrText" name="rcvrPostAddrText" value="${orderInfo.rcvrPostAddrText}"/>
			<input type="hidden" id="rcvrDtlAddrText" name="rcvrDtlAddrText" value="${orderInfo.rcvrDtlAddrText}"/>
			...
			<%-- 2020.05.07: 주문상세에서 넘어오는 비회원 AS반풉접수 여부  --%>
			<input type="hidden" id="asReturnYn" name="asReturnYn" value="${asReturnYn}"/>
		
			<!-- S : content-header -->
			<div class="content-header">
				<div class="fl">
					<h3 class="content-title"><a href="#" class="link"><span>${orderInfo.loginId}</span> (<span>${orderInfo.buyerName}</span>)</a>님의 반품접수 정보</h3>
				</div>
			</div>
			<!-- E : content-header -->
			...
			<div class="tbl-controller">
				<div class="fl">
					<span class="opt-group">
						...
						<select class="ui-sel" id="codeDivSelect">
						...
						<select id="clmRsnCodeSelect" class="ui-sel" title="반품유형 선택" style="display:none;">
					...
					<span class="btn-group">
						...
						<span class="btn-sm" id="returnNotice">※ 반품사유가 '오프라인 반품', '미출반품' 이면 택배사 수거지시는 내려지지 않습니다.</span>
					</span>
				</div>
			</div>
			<!-- E : tbl-controller -->
					...
					<c:set var="loopIdx" value="${status.index}"/>
						<tr>
							...
							<td class="input">
								...
								<textarea class="ui-textarea" title="반품사유 입력" name="clmEtcRsnText"></textarea>
							</td>
							<td class="input">
							...
							<tr>
								...
								<td class="input">
									...
									<ul class="ip-box-list">
										...
										<%-- 2020.03.05 : 주결제수단이 휴대폰결제일 경우 환불금 차감 안되어야한다. --%>
										...
										<%-- 2020.03.05 : 주결제수단이 휴대폰결제일 경우 선결제 기본 체크 --%>
										<li>
											<span class="ui-rdo">
												<!-- DESC : input id / label for 동일하게 맞춰주세요 -->
			...
			<div class="window-btn-group">
				...
				<a href="#" class="btn-normal btn-func" id="claimRegistBtn">반품접수</a>
			</div>
			<!-- E : window-btn-group -->
...
<script src="/static/common/js/biz/claim/abcmart.claim.return.create.js<%=_DP_REV%>"></script>
