...
(function() {
	...
	***************************************************************************/
	...
	var _emailtemplate = abc.object.createNestedObject(window,"abc.biz.cmm.email.manual.template");
	...
	_emailtemplate.init = function() {
		...
		if(Object.keys(gridContainer).length !== 0) {			// main.jsp 일때
			_emailtemplate.initEmailTemplateSheet();			// 그리드 생성
			_emailtemplate.mainEvent();							// 메인화면 이벤트 핸들러 생성
			_emailtemplate.moveDetail();						// 상세페이지 핸들러 생성
	...
	// 이메일 템플릿 목록 그리드
	_emailtemplate.initEmailTemplateSheet = function() {
	...
	// 메일 템플릿 관리 Action 처리
	_emailtemplate.doActionEmailTemplate = function (sAction) {
		var _url = "/cmm/msg/email-template/manual/";
		var _method	= "";
			...
			case "save" : // 저장
				...
				$.ajax({
					...
					if (data.code == abc.consts.BOOLEAN_TRUE) {
						if (opener != null) {
							opener.parent.abc.biz.cmm.email.manual.template.doActionEmailTemplate("search");
							window.self.close();
						}
					}
	...
	_emailtemplate.mainEvent = function() {
		...
		$("#searchBtn").click(function(f) {
			_emailtemplate.doActionEmailTemplate("search");
		});
		...
		$("#rgstBtn").click(function(f) {
			abc.open.popup({
				url 	:	"/cmm/msg/email-template/manual/read-detail-pop",
	...
	_emailtemplate.detailEvent = function() {
		...
		$("#saveBtn").click(function(f) {
			_emailtemplate.doActionEmailTemplate("save");
		});

		$("#deleteBtn").click(function(f) {
			_emailtemplate.doActionEmailTemplate("delete");
		});
