...
(function() {
	var _settlementmonth = abc.object.createNestedObject(window,"abc.biz.settlement.month.main");

	/**
	 * ibsheet 초기화
	 */
	_settlementmonth.sheetInit1 = function(){
	...
	/**
	 * 패널티 조정 금액 및 정산조정금액 이벤트 
	 */
	monthSheet_OnChange = function(Row, Col, Value){
		
		var penltyMdatAmt = monthSheet.GetCellValue( Row , "penltyMdatAmt") //해당 row
		var excclcMdatAmt = monthSheet.GetCellValue( Row , "excclcMdatAmt") //해당 row

		var ColCheckedRowsNumber = monthSheet.SaveNameCol("checkedRows");  //체크 박스 Col번호
		
		//체크박스는 영향을 안받게 하기위해
		if( Col != 1 ){
			if( abc.text.allNull( penltyMdatAmt) &&  abc.text.allNull( excclcMdatAmt)  ){
				monthSheet.SetCellValue(Row, ColCheckedRowsNumber, 0);  //체크박스 체크처리
			}else{
				monthSheet.SetCellValue(Row, ColCheckedRowsNumber, 1); 
			}
		}//end if Col
	}
	 ...
	 * 정산 조정 금액에 대한 수정 
	 */
	_settlementmonth.doExcclcMdatPymntMod = function(){
 
	//선택여부 체크 * 형식이 있기에 두형식으로 사용 해봄
	var sRow = monthSheet.CheckedRows("checkedRows");
	 
	 //선택 데이타 Rows
	var checkRows = monthSheet.FindCheckedRow("checkedRows");
	...
	for (idx=0; idx<arrRow.length; idx++){  
		var penltyMdatAmt =  monthSheet.GetCellValue( arrRow[idx], "penltyMdatAmt");		 //이전 배송상태
		var excclcMdatAmt =  monthSheet.GetCellValue( arrRow[idx], "excclcMdatAmt");		 //배송ID
		var excclcSeq =  monthSheet.GetCellValue( arrRow[idx], "excclcSeq");		 //배송상태
	}
	// save
	if(confirm(alertConfirmMod)){
		 var param = {
						url : "/settlement/settlement-excclc-payment-mdat",
	 ...
	 * ibsheet 초기화
	 */
	_settlementmonth.sheetInit2 = function(){
	 ...
	 * ibsheet 초기화
	 */
	_settlementmonth.sheetInit3 = function(){
...
//		dlvyAmtSheet.SetFrozenCol(1);
	 ...
	 * ibsheet 초기화
	 */
	_settlementmonth.sheetInit4 = function(){
	 ...
	 * ibsheet 초기화
	 */
	_settlementmonth.sheetInit5 = function(){
	 ...
	 */
	_settlementmonth.event = function(){
		/**
		 * 엑셀 다운로드
		 */
		...
		$('#month, #sellAmt,#dlvyAmt,#promoAmt,#penlty').on("click", function(){
			if(this.id == 'month'){
				...
				$('#tabGubun').val('1');
				_settlementmonth.monthOptionRander();
			}else{
				if(this.id == 'sellAmt'){
					$('#tabGubun').val('2');
		 ...
		 * 이벤트 바인딩
		 */
		_settlementmonth.orderNoSelect();
		 ...
		 * 저장 버튼 이벤트 바인딩
		 */
		$("#excclcModify").click(function(){ 
			 _settlementmonth.doExcclcMdatPymntMod();
		});
	 ...
	 * 그리드 데이터 조회
	 */
	_settlementmonth.getList = function(){
		var tabGubun 	= $('#tabGubun').val();
		...
		// 탭 별로 정산내역 조회 
		_settlementmonth.SettlementHistory(tabGubun);
	}
	
	/**
	 * 탭에 해당하는 정산내역을 조회 
	 */
	_settlementmonth.SettlementHistory = function(tabGubun){
		$.ajax({
	 ...
	 * 검색어 
	 */
	_settlementmonth.orderNoSelect = function(){
	...
	$("#btnBrand").click(function(){
		...
		abc.brandSearchPopup(false,"abc.biz.settlement.month.main.setSelectedBrand");
	});
	
	/**
	 * 브랜드 찾기 callback 이벤트
	 */
	_settlementmonth.setSelectedBrand = function(data) {
	 ...
	 * 정산확정
	 */
	$("#excclcConfirm").click(function(){
		if($('#excclcDcsnYn').val() == 'Y'){
			alert('정산확정 처리된 건입니다.');
		}else{
			$.ajax({
		        url: '/settlement/settlement-confirmation',
		        type: 'post',
		        data:  $("#gForm").serialize(),
		        success:function(data){
		        	if(data.result == 'Y'){
...
//	    });
	 ...
	 * 관리자 메모 등록
	 */
	_settlementmonth.createMemo = function(){	
		var excclcSeq = $("#excclcSeq").val();
		...
		if(abc.text.isLimitLength(memoText,500)){
			alert("메모에 글자를 초과하였습니다.");
			$("#memoText").focus();
			return false;
		}
		if(excclcSeq == "0"){
			alert("정산 데이타가 존재하지 않는 "+year+"년"+month+"월"+"입니다.\n 확인후 다시 시도해주세요.");
			$("#memoText").val('');
			return false;
		}
		$.ajax({
			type :"post",
			url : "/settlement/create-settlement-memo",
			data : {"excclcSeq" : excclcSeq, "memoText" : memoText},
		}).done(function(data){
			var chrHtml;
			var rgstDtm = new Date(data.memoInfo[0].rgstDate);
			chrHtml  = "<li>";
			chrHtml += "<span class='msg-list-info'>";
			chrHtml += "<span class='user-info'>";
			chrHtml += "<span class='user-id'>"+ data.memoInfo[0].loginId  +"</span>";
			...
			chrHtml += "<input type='hidden' value='"+ data.memoInfo[0].excclcMemoSeq +"'>";
	 ...
	 * 관리자 메모 삭제
	 */
	_settlementmonth.removeMemo = function(idx, classNm) {
		var excclcMemoSeq = $("."+classNm).eq(idx).find("input").val();
		var excclcSeq = $("#excclcSeq").val();
		
		$.ajax({
			type :"post",
			url : "/settlement/update-settlement-memo",
			data : {"excclcSeq" : excclcSeq, "excclcMemoSeq" : excclcMemoSeq},
		}).done(function(data){
	 ...
	 */
	$(function(){
		_settlementmonth.sheetInit1();
		_settlementmonth.sheetInit2();
		_settlementmonth.sheetInit3();
		_settlementmonth.sheetInit4();
		_settlementmonth.sheetInit5();
		_settlementmonth.event();
//		_settlementmonth.sheet();
	});
	 ...
	 */
	_settlementmonth.monthOptionRander = function(){
		var th = monthOption;
