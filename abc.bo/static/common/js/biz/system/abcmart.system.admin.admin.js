 ...
 */
(function() {
	var _admin = abc.object.createNestedObject(window,"abc.biz.system.admin.admin");
	...
	_admin.adminDoAction = function(sAction){
		switch(sAction) {
			...
			case "search" :
				...
				var param = { url : "/system/admin/admin/read-list"
	 ...
	 * 관리자 등록화면 호출
	 */
	_admin.registAdmin = function(){
		var url = "/system/admin/admin/create-pop";
		var params = {}
	...
	_admin.authGroupUpdate = function(){
		...
		var url = "/system/admin/admin/update-admin-authority-pop";
		var params = {}
...
//		$("#searchValue").val('');
	...
	_admin.authGroupChange = function(authNo){
		...
		$.ajax({
			type :"post",
			url : "/system/admin/admin/read-authority",
			data : {"authNo" : authNo},
		})
		.done(function(data){
			if(authNo != ""){
				...
				if(authSystemType == "B"){
					$("#authSystemType").text(abc.consts.AUTH_TYPE_BO);
					$("#accessIpArea").show();
				}else{
					$("#authSystemType").text(abc.consts.AUTH_TYPE_PO);
					$("#accessIpArea").hide();
				}
	...
	_admin.joinAdmin = function(){
		...
		var authSystemType = $("#authSystemType").text();
		...
		if(authSystemType == abc.consts.AUTH_TYPE_BO){
			if(ipItemCnt < 1){
		...
		if(confirm("저장하시겠습니까?")){
			...
			.done(function(data){
				...
				opener.abc.biz.system.admin.admin.adminDoAction('search');
				window.self.close();
			})
			.fail(function(e){
	...
	_admin.adminAuthorithDoAction = function(sAction){
		...
		case "update" :
			...
			if(confirm("[" + authGroupNm + "]" + "으로 변경하시겠습니까? ")){
				var param = { url:"/system/admin/admin/update-admin-authority"
					, subparam:FormQueryStringEnc(document.frmSearch)
					, sheet:"adminAuthoritySheet"};
	...
	_admin.tabHandler = function(event, ui) {
		...
		if (ui.newTab.prevObject.attr('id') === "tabLogin") {
			if (typeof channelSheet === "undefined") {
				abc.biz.system.admin.admin.initLoginSheet();
			}
		}else if(ui.newTab.prevObject.attr('id') === "tabHistory"){
			if (typeof channelSheet === "undefined") {
				abc.biz.system.admin.admin.initChangeHistorySheet();
	...
	/**
	 * 화면 로드 시 접근허용 아이피 영역 셋팅
	 */
	_admin.detailAccessIpAreaSet = function(authSysType){
		if(authSysType == abc.consts.AUTH_TYPE_BO){
			$("#accessIpArea").show();
		}else{
			$("#accessIpArea").hide();
		}
	...
	_admin.pswdReset = function(){
		...
		if(confirm("비밀번호를 초기화 하시겠습니까?")){
			$.ajax({
				type :"post",
				url : "/system/admin/admin/update-pswd-reset",
	...
	_admin.updateAdmin = function(){
		...
		if(authSystemType == abc.consts.AUTH_TYPE_BO){
			...
			for(var i=0; i < ipItemCnt; i++){
				if(i == 0){
					ipItems = $(".item-list > li > span").eq(i).text();
				}else{
					ipItems += "," + $(".item-list > li > span").eq(i).text();
				}
			}
	
			// 변경이력  과거데이터와 비교하기 위해 세팅
			$("#accessIpText").val(ipItems);
		}

		if(confirm("저장하시겠습니까?")){
			$.ajax({
				type :"post",
				url : "/system/admin/admin/update",
			...
			.done(function(data){
				if(data.result == "Y"){
					...
					opener.abc.biz.system.admin.admin._CPage = opener.adminSheet.GetCurrentPage();
					opener.abc.biz.system.admin.admin.adminDoAction('search');
					opener.abc.biz.system.admin.admin._CPage = 1;
	...
	_admin.DetailDoAction = function(sAction){
		switch(sAction) {
			...
			case "loginSearch" :
				...
				var param = { url : "/system/admin/admin/read-admin-use-history"
					, onePageRow : pageCount
			...
			case "changeSearch" :
				...
				var param = { url : "/system/admin/admin/read-admin-change-history"
					, onePageRow : pageCount
	 ...
	 *							A-Connect 사용자 관리
	 *************************************************************************/
	_admin.initAconnectAdminList = function() {

		// Grid 객체 생성 //
		createIBSheet2(document.getElementById("aconnectAdminGrid"), "aconnectAdminSheet", "100%", "420px");
		...
		initSheet.Cols = [
			...
			, {Header:"사용자번호", 		Type:"Text",		SaveName:"adminNo",				Width: 10, Align:"Center", 	Edit:0, Hidden:1}
			, {Header:"번호", 			Type:"Seq",			SaveName:"",					Width: 10, Align:"Center", 	Edit:0, Sort:0}
			, {Header:"사용자ID",		 	Type:"Text",		SaveName:"listAconnectId",		Width: 20, Align:"Center", 	Edit:0, Sort:0}
			, {Header:"사용자명",	 		Type:"Text",		SaveName:"listAconnectName",	Width: 20, Align:"Center", 	Edit:0, FontBold:1, FontUnderline:1, Cursor:"Pointer", Sort:0}
			, {Header:"사용자권한그룹",		Type:"Text",		SaveName:"authName",			Width: 20, Align:"Center", 	Edit:0, Sort:0}
			, {Header:"사용자권한매장",		Type:"Text",		SaveName:"storeName",			Width: 25, Align:"Center", 	Edit:0, Sort:0}
			, {Header:"사용여부", 			Type:"Combo",		SaveName:"useYn",				Width: 15, Align:"Center", 	Edit:0, Sort:0}
			, {Header:"관리자등록일시",		Type:"Date",		SaveName:"rgstDtm",				Width: 30, Align:"Center", 	Edit:0, Format:abc.consts.DEFAULT_DATETIME_PATTERN}
		];

		// Grid 초기화
		IBS_InitSheet(aconnectAdminSheet , initSheet);

		// Grid 건수 정보 표시
		aconnectAdminSheet.SetCountPosition(3);

		// Grid 페이지 네비게이션 버튼 표시
		aconnectAdminSheet.SetPagingPosition(2);

		// Grid width 자동 조절
		aconnectAdminSheet.FitColWidth();

		// Grid 마지막 컬럼을 All 너비에 맞춘다.
		aconnectAdminSheet.SetExtendLastCol(1);

		aconnectAdminSheet.InitDataCombo(0 , "useYn" , "사용|사용안함" , "Y|N");
	}

	_admin.doActionAconnectAdmin = function(sAction) {
		var _url = "/system/admin/store/";
		var _method = "";
		switch(sAction) {
		case "search" :
			_method = "read-list";
			...
			var param = { url : _url + _method
				, onePageRow : pageCount
				, subparam : FormQueryStringEnc(document.frmSearch)
				, sheet : "aconnectAdminSheet" };

			DataSearchPaging(param);

			break;
		case "save" :	// AConnect 관리자 등록
			if(abc.text.isBlank($("#adminNo").val())) {
				_method = "create";
			} else {
				_method = "update";
			}
			...
			if (_method == "create") {
				...
				if(abc.text.isBlank(confirmLoginId)){
					alert("사용자 ID 중복확인을 해야합니다.");
					return false;
				}else if(confirmLoginId > 0){
					alert("중복 ID는 사용할 수 없습니다.");
					return false;
				...
				// 비밀번호 확인 유효성 체크
				if(!window.abc.connectCheckConfirmPassword()){
					return false;
				}
			}
			...
			if(abc.text.isBlank(hdphnNoText)){
				alert("휴대폰 번호를 입력해 주세요.");
				$("#hdphnNoText").focus();
				return false;
			}
			...
			var storeCnt = $("#storeTr").find("span").length;
			if(storeCnt <= 0){
				alert("매장을 등록해야 합니다.");
				return false;
			}
			...
			if(confirm("저장하시겠습니까?")){
				$.ajax({
					type :"post",
					url : _url + _method,
					data: $("#frmAconnectAdmin").serialize()
				}).done(function(data, textStatus, jqXHR) {
					if(data.result == abc.consts.BOOLEAN_TRUE){
						alert("저장되었습니다.");
						location.href = _url + "?menuNo=" + abc.param.getParams().menuNo;
					}
